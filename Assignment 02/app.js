...
searchButton.addEventListener("click", () => {
  ...
  if (selectedBrand && selectedModel) {
      ...
      let detailsHTML = `<h3>${selectedBrand.toUpperCase()} - ${selectedModel.toUpperCase()}</h3>`;

      // Helper function to display object details
      function displayObjectDetails(obj, indent = 0) {
          let html = "";
          for (const key in obj) {
              if (typeof obj[key] === "object" && obj[key] !== null) {
                  html += `<div style="margin-left: ${indent}px;"><strong>${key}:</strong></div>`;
                  html += displayObjectDetails(obj[key], indent + 20); // Recursive call
              } else {
                  html += `<div style="margin-left: ${indent}px;"><strong>${key}:</strong> ${obj[key]}</div>`;
              }
          }
          return html;
      }

      detailsHTML += displayObjectDetails(mobileDetails);
